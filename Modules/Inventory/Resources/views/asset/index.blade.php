        ...
        <div class="col-xs-12">
          <div class="box">
            ...
            <div class="box-body">
              <div class="col-md-12">
			    <strong>
			    	Asset Dari Nomor Mutasi IN = {{ $assets[0]->inventarisirDetail->inventarisir->no }}
			    </strong>
			    <br/>
			    <strong>
			    	{{ $assets[0]->item->name." = ".$assets->sum('quantity')." ".$assets[0]->satuan->name }}
			    </strong>
				<form action="{{ url('/inventory/asset/create_qrCode') }}" name="form-qrcode" id="form-qrcode" method="post">
					{{ csrf_field() }}
					<input type="hidden" name="inventarisir_detail_id" id="inventarisir_detail_id" value="{{ $assets[0]->inventarisir_detail_id }}"/>
				...
				@include('form.a',
							[
								'href' => url('/inventory/inventarisir_detail/index').'?id='.$assets[0]->inventarisirDetail->inventarisir_id,
								'class'=>'pull-right',
								'caption' => 'Kembali'
							])
				<br/>
...
<script type="text/javascript">
	...
	{
		return '<a href="#" class="editable_header" data-pk="'+row.id+'" data-name="to_user_id" data-url="{{url("/inventory/asset/update")}}" data-original-title="Pemegang" data-type="select" data-source="{{ url("/general/user_source") }}" data-value="'+data+'">'+data+'</a>';
	}

	var fnEditableRoom = function(data,type,row,meta)
	{
		return '<a href="#" class="editable_header" data-pk="'+row.id+'" data-name="to_room_id" data-url="{{url("/inventory/asset/update")}}" data-original-title="Penempatan" data-type="select" data-source="{{ url("/general/room_source") }}" data-value="'+data+'">'+data+'</a>';
	}
  ...
  $.fn.editable.defaults.mode = 'inline';
	...
	$(document).ready(function()
	{
		...
		gentable.on('draw',function()
		{
		    ...
		    /*$('.editable_user').editable({
		    	source : '/general/user_source',
		    	select2: {
		    ...
		    $('.editable_place').editable({
		    	source : '/general/room_source',
		    	select2: {
		            placeholder: 'Pilih Pemegang',
		        },
		    });*/
