...
class ItemController extends Controller
{
    ...
    public function add(Request $request)
    {
        ...
        return view('inventory::items.add_form', compact('item_categories','brands','warehouses','user','project'));
    }
    ...
    {
        $items                      = \App\Item::find($request->id);
        $item_categories            = ItemCategory::get();
        $subcategories = ItemCategory::find($items->item_category_id)->child()->get();
        $warehouses                 = Warehouse::get();
        
        return view('items.edit_form', compact('items', 'item_categories', 'warehouses','subcategories'));
    }
    ...
    public function getItems(Request $request)
    {
        ...
        $items = Item::select('items.id as item_id','items.item_category_id','items.name as item_name','items.stock_min','cte.sat')
        ->leftJoin(
            ...
            'items.id','=','cte.id_it')
        ->groupBy('items.id','item_category_id','items.name','items.stock_min','cte.sat')->get();
        ...
        foreach ($items as $key => $value){
            ...
            $arr = array(
                'id' => $value->item_id,
                'item_category'=> $value->category->name,
    ...
    public function details($id)
    {
        ...
        return view('inventory::items.details_item',compact('results','stockResults','resultSatuans','booking','projectname'));
